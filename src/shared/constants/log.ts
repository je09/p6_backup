export const LOG_MESSAGES = {
  ...
  MODE_SWITCH_COMPLETE: (mode: string) =>
    `Mode switch complete. New mode: ${mode}`,
  RAW_DEVICE_MODE: (rawMode: string, mappedMode: string) =>
    `Raw device mode: ${rawMode}, mapped to: ${mappedMode}`,
  ...
  CREATING_BACKUP_FOLDER: (folderName: string) =>
    `Creating backup folder: ${folderName}`,
  WRITING_MANIFEST: (fileName: string) => `Writing manifest file: ${fileName}`,
  COPYING_PATTERNS: "Copying pattern data to backup folder",
  COPYING_SAMPLES: (bankId: string) =>
    `Copying samples from bank ${bankId} to backup folder`,
  BACKUP_COMPLETE: (folderName: string) =>
    `Backup completed successfully in folder: ${folderName}`,
  STARTING_RESTORE: "Starting restore operation...",
  OPERATION_COMPLETE: (type: string) =>
  ...
  OPERATION_FAILED: (type: string, error: string) =>
  ...
  FOUND_SAMPLES: (count: number, bank?: string) =>
    bank ? `Found ${count} samples in bank ${bank}` : `Found ${count} samples`,
  ...
  UPDATED_INFO_FILE: (count: number) =>
    `Updated info file with ${count} entries`,
  SAMPLE_READ_ERROR: (bankId: string, error: any) =>
    `Error reading samples from bank ${bankId}: ${error}`,
} as const;
