export const ERROR_MESSAGES = {
    ...
    DEVICE_NOT_FOUND: 'Roland P6 device not detected. Please ensure the device is connected and in the correct mode.',
    
    // Mode errors
    MODE_SWITCH_FAILED: 'Mode switch failed',
    MODE_SWITCH_TIMEOUT: 'Timeout waiting for device mode switch. Please ensure device is in the correct mode and try again.',
    MODE_DETECTION_FAILED: 'Failed to detect required device mode. Please check device connection and mode.',
    DEVICE_MODE_INVALID: 'Device must be in the correct mode for this operation',
    
    // Backup errors
    BACKUP_FAILED: 'Backup operation failed',
    PATTERN_BACKUP_FAILED: 'Pattern backup failed',
    SAMPLE_BACKUP_FAILED: 'Sample backup failed',
    FULL_BACKUP_FAILED: 'Full backup failed',
    BACKUP_FOLDER_NOT_FOUND: 'Backup folder not found on the device. Please verify the device is in backup mode.',
    BACKUP_FOLDER_EMPTY: 'The backup folder on the device is empty or inaccessible.',
    ...
    RESTORE_FOLDER_NOT_FOUND: 'Restore folder not found on the device. Please verify the device is in restore mode.',
    INVALID_BACKUP_FILE: 'Invalid backup file or corrupted data.',
    BANK_NOT_FOUND_IN_BACKUP: (bankId: string) => `Bank ${bankId.toUpperCase()} not found in backup`,
    ...
    FAILED_TO_READ_BANK_DATA: (bankId: string) => `Failed to read bank ${bankId} data`,
    ...
    COULD_NOT_READ_SAMPLES: (bankId?: string) => 
        bankId ? `Could not read samples from bank ${bankId}` : 'Could not read all samples',
    ...
    DEVICE_EJECT_FAILED: 'Failed to eject P6 device safely. You may manually disconnect the device.',

    // Combined backup validation
    COMBINED_BACKUP_DEVICE_DISCONNECTED: 'Device must be connected to proceed with backup',
    COMBINED_BACKUP_WRONG_PATTERN_MODE: (currentMode: string) => 
        `Device is in '${currentMode}' mode. Please switch to Pattern mode (hold PLAY button while powering on) and try again.`,
    COMBINED_BACKUP_WRONG_SAMPLE_MODE: (currentMode: string) => 
        `Device is in '${currentMode}' mode. Please switch to Sample mode (hold BANK + SAMPLING buttons while powering on) and try again.`,
    COMBINED_BACKUP_WRONG_BANK: (deviceBank: string, targetBank: string) => 
    ...
    COMBINED_BACKUP_BANK_NOT_AVAILABLE: (targetBank: string, availableBanks: string[]) => 
        `Bank ${targetBank.toUpperCase()} is not available on the device. Available banks: ${availableBanks.join(', ')}. Please check your device setup.`,
} as const;
...
export const SUCCESS_MESSAGES = {
    ...
    PATTERN_BACKUP_COMPLETED: 'Pattern backup completed successfully!',
    SAMPLE_BACKUP_COMPLETED: 'Sample backup completed successfully!',
    FULL_BACKUP_COMPLETED: 'Full backup completed successfully!',
    PATTERNS_BACKED_UP: (count: number) => `Successfully backed up ${count} patterns`,
    ...
    PATTERNS_RESTORED: (count: number) => `Successfully restored ${count} patterns`,
    ...
    BANK_COPIED: (bankId: string) => `Copied bank ${bankId.toUpperCase()} to IMPORT folder`,
    PATTERNS_WRITTEN: (count: number) => `Successfully wrote ${count} patterns to device`,
    ...
    SAMPLE_PROCESSED: (sampleName: string, padNumber: string) => 
        `Successfully processed sample ${sampleName || 'unnamed'} from pad ${padNumber}`,
    INFO_FILE_UPDATED: (count: number) => `Updated info.txt with ${count} entries`,
    ...
    DEVICE_EJECTED: 'P6 device ejected safely. You can now disconnect the device.',
} as const;
...
export const STATUS_MESSAGES = {
    ...
    BACKING_UP_SAMPLE_BANK: (bankId: string) => `Backing up sample bank ${bankId.toUpperCase()}...`,
    RESTORING_PATTERNS: 'Restoring patterns...',
    RESTORING_SAMPLES: 'Restoring all sample banks...',
    RESTORING_SAMPLE_BANK: (bankId: string) => `Restoring sample bank ${bankId.toUpperCase()}...`,
    PERFORMING_FULL_BACKUP: 'Performing full backup...',
    FULL_BACKUP_COMPLETED_STATUS: 'Full backup completed successfully!',
    ...
    FOUND_SAMPLES_ALL_BANKS: (count: number) => `Found ${count} samples across all banks`,
...
export const INFO_MESSAGES = {
    // Device requirements
    DEVICE_MUST_BE_CONNECTED: 'Device must be connected to perform restore operations',
    DEVICE_MUST_BE_CONNECTED_FOR_PATTERN_RESTORE: 'Device must be connected for pattern restore',
    DEVICE_MUST_BE_CONNECTED_FOR_SAMPLE_RESTORE: 'Device must be connected for sample restore',
...
export const LOG_MESSAGES = {
    ...
    CAN_BACKUP_SAMPLES: (canBackup: boolean) => `Can backup samples: ${canBackup}`,
    
    // Operation logging
    READING_DATA: (dataType: string, parameters?: any) => 
        `Reading data: ${dataType}${parameters ? ` ${JSON.stringify(parameters)}` : ''}`,
    WRITING_DATA: (dataType: string, data: any, parameters?: any) => 
        `Writing data: ${dataType}${parameters ? ` ${JSON.stringify(parameters)}` : ''}`,
    BACKUP_SAMPLES_READ_TYPE: (bankId: string, type: string, isArray: boolean) => 
        `backupSamples: read samples from bank ${bankId}, type: ${type} ${isArray ? 'array' : 'not array'}`,
    ...
    PROCESSING_SAMPLE: (sample: any) => `Processing sample: ${JSON.stringify({
        pad: sample.pad,
        name: sample.name,
    ...
    COPY_SAMPLES_INPUT_TYPE: (type: string, isArray: boolean, isNull: boolean) => 
        `copySampleFiles - input samples type: ${type} ${isArray ? 'array' : 'not array'} ${isNull ? 'null' : 'non-null'}`,
    READ_SAMPLES_RESULT_TYPE: (bankId: string, type: string, isArray: boolean) => 
        `readSamplesFromBank ${bankId} result type: ${type} ${isArray ? 'array' : 'not array'}`,
    
    // Warning messages
    SKIPPING_NULL_SAMPLE: 'Skipping null/undefined sample',
    UNEXPECTED_SAMPLES_FORMAT: (format: string) => `Expected samples to be an array or object, but got: ${format}`,
    UNEXPECTED_BANK_SAMPLES_FORMAT: (format: string) => `Expected bankSamples to be an array, but got: ${format}`,
    ...
    GOT_ARRAY_NO_BANK_ID: 'Got an array of samples but no bankId specified. Using \'unknown\' as bank.',
    ALL_BANKS_UNEXPECTED_FORMAT: (type: string, isArray: boolean) => 
        `All banks read returned unexpected format: ${type} ${isArray ? 'array' : 'not array'}`,
...
export const OPERATION_NAMES = {
    ...
    SAMPLE_BACKUP_BANK: (bankId: string) => `Sample Backup (Bank ${bankId.toUpperCase()})`,
    SAMPLE_RESTORE_BANK: (bankId: string) => `Sample Restore (Bank ${bankId.toUpperCase()})`,
    SAMPLE_BACKUP_ALL: 'Sample Backup (All Banks)',
    SAMPLE_RESTORE_ALL: 'Sample Restore (All Banks)',
...
export const UI_LABELS = {
    ...
    BACKUP_ALL_BANKS: 'Backup All Banks',
    ...
    // Mode instructions
    MODE_INSTRUCTION_PATTERN: 'Switch your P6 to Pattern mode by pressing the PLAY button.',
    MODE_INSTRUCTION_SAMPLE: 'Switch your P6 to Sample mode by pressing the SAMPLE button.',
    ...
    BANK_SWITCHING_INSTRUCTION: (bankId: string) => `Hold BANK ${bankId.toUpperCase()} + SAMPLING buttons while powering on`,
    BACKING_UP_BANK: (bankId: string) => `Backing up Bank ${bankId.toUpperCase()}`,
    ...
    CONNECTED_MODE: (mode: string) => `Connected - ${mode.charAt(0).toUpperCase() + mode.slice(1)} Mode`,
    BANK_INFO: (bank: string) => ` (Bank ${bank})`,
    BANKS_INFO: (banks: string[]) => ` (Banks: ${banks.join(', ')})`,
